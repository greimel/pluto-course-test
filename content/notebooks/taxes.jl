...
# ╔═╡ 29b44172-f5fd-4db5-8cba-ef415624e801
using PlutoUI: Slider

# ╔═╡ fa83be20-c311-495e-bcff-ab11ec1ab001
using CairoMakie

# ╔═╡ f0a01c75-c964-4e3e-82da-8e4803ec672a
begin
	using LaTeXStrings
...
# ╔═╡ 609294d2-4df5-4d91-b412-89ca6d871261
let
	fig = Figure()
	ax = Axis(fig[1,1])#, ylims = [0, 1.1 * α_D])
	
	
	lines!(ax, q_grid, P_D.(q_grid), color=:blue, label="Demand")
	lines!(ax, q_grid, P_S.(q_grid), color=:green, label="Supply")
	hlines!(ax, [p0], label = "prices", color=:red, linestyle=:dash, alpha=0.5)
	hlines!(ax, [PDτ], color=:red, linestyle=:dash, alpha=1)
	hlines!(ax, [PSτ], color=:red, linestyle=:dash, alpha=1)

	
	vlines!(ax, [q0], label = "quantity", color=:orange, linestyle=:dash, alpha=0.5)
	vlines!(ax, [qτ], color=:orange, linestyle=:dash, alpha=1)
	...
	band!(ax, 
		...
		fill(PDτ, 20),
		P_D.(zero2q),
		label="consumer surplus $(round(CSurτ, digits=1))", 
		color = (:blue, 0.5),
#		alpha = 0.5,
	)
	band!(
		...
		fill(PSτ, 20), 
		P_S.(zero2q),
		label="producer surplus $(round(PSurτ, digits=1))",
		color = (:green, 0.5)
	)
	band!(
		zero2q,
		fill(PSτ, 20),
		fill(PDτ, 20),
		label="tax revenue $(round(TR, digits=1))",
		color = :yellow
	)
