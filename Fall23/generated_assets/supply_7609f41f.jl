...
# ╔═╡ 4a2c6c86-a574-405c-afe1-49dbb93e35e8
using PlutoUI

# ╔═╡ cfb60bfa-221a-40fa-96dd-51c63c14eeaa
using PlutoUI: Slider

# ╔═╡ 81f9edca-2424-11eb-0f7f-bba3aa6bf064
using CairoMakie

# ╔═╡ 8b2389be-fb8c-4eb6-97db-1865854aad1c
begin
	using LaTeXStrings
...
let
	fig = Figure()
	ax = Axis(fig[1,1])
	lines!(0..3, π_long,  label="long-run")
	lines!(0..3, π_short, label="short-run")
...
# ╔═╡ b560e5ae-9622-4b2e-837c-6b22295f5532
print_point(p) = repr(Float64.(collect(round.(p, digits=2))))
...
begin
	...
	x₂_max = max(8, budget_line(0, c)[2] + 0.1)
...
let
	...
	contour!(ax, x₁_grid, x₂_grid, y_grid,
		color=:gray, 
		labels=true, alpha = 0.5, levels=10
		)
	contour!(ax, x₁_grid, x₂_grid, y_grid,
		color=:green, labels=true, levels = [y]
	)
	lines!(ax, budget_line.([0, c/w₁], c), color=:red, label="cost $c")

	ip = intersection_points(c, y)
	
	scatter!(ax, ip,
		color = :blue, label = "possible inputs"
	)
